...
function ArticleCard({ title, duration, content, date, image }:ArticleType) {
    return (
        <div className="p-2 bg-white rounded-2xl border-[#FBFBFB] border">
            <div className="relative">
                <img src={image} alt="article cover image" className="object-cover aspect-video h-64"/>
                <motion.div initial={{ opacity: 0 }} animate={{ y: '10px'}} whileHover={{ opacity: 1, y: '0px' }} className="absolute bottom-0 inset-x-0 text-center">
                    <p className="mt-[14px] mb-3 font-OpenSans text-E2E font-semibold text-xl capitalize">{title.toLocaleLowerCase().substring(0, 100)}</p>
                </motion.div>
                {/* <motion.div whileHover={{ opacity: 0.3 }} className="absolute inset-0 -z-10" /> */}
            </div>
            <div className="flex items-center mt-6 font-OpenSans font-semibold text-xs tracking-0.04 text-9D9 capitalize">
            ...
            {/* <p className="mt-[14px] mb-3 font-OpenSans font-semibold text-xl capitalize text-222">{title.toLocaleLowerCase().substring(0, 100)}</p> */}
            <p className="font-OpenSans my-3 tracking-0.02 capitalize text-747">{content.substring(0,200)}</p>
            <p className="cursor-pointer text-sm mt-4 font-medium text-purple-600 hover:text-purple-800 dark:text-purple-500 dark:hover:text-purple-700">Read More..</p>
        </div>
    )
}
